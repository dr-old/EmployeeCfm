...
const TopupConfirm = () => {
  ...
  return (
    <Container
      ...
      <View style={stylesCust.card}>
        <View style={stylesCust.cardTitle}>
          <Text style={styles.h5(color.tblack)}>Select Payment Method</Text>
        </View>
        <Divider height={10} />
        {topup?.paymentMethod?.id ? (
          <TilePayment
            item={topup.paymentMethod}
            onClick={() => navigation.push('PaymentMethod')}
          />
        ) : (
          <TouchableOpacity
            onPress={() => navigation.push('PaymentMethod')}
            style={stylesCust.cardNominal()}>
            <Text style={styles.p4(color.tblack)}>
